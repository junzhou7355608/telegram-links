...
export function buildAiContext(
  ...
  return {
    ...
    current: toContextMessage(message, 4096),
    forwardSource: message.context.forwardSource ?? null,
    neighbors: [
      ...message.context.previous.map((value) => toContextMessage(value, 1000)),
      ...(message.context.next
        ? [toContextMessage(message.context.next, 1000)]
        : []),
    ],
    reply: message.context.reply
      ? toContextMessage(message.context.reply, 1000)
      : null,
  };
}
